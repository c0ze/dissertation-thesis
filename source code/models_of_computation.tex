...
Such machines can solve some problems efficiently with bounded or negligible error, even when the best known deterministic exact algorithms are much more expensive. Bounded error does not enlarge the class of computable functions --- anything a probabilistic Turing machine can decide with bounded error is already computable by a deterministic machine --- but it can dramatically change the \emph{resources} required. Probabilistic computation with negligible error is the right reference class for quantum computation, whose output is also sampled rather than read out exactly.
