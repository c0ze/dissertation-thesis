...
void apply_modular_exp(qreg *q, int counting_start, int t,
    ...
    for (size_t idx = 0; idx < q->local_size; idx++) {
        ...
        size_t x = (global >> counting_start) & ((1ULL << t) - 1);
        size_t y = (global >> target_start)   & ((1ULL << n) - 1);
        size_t y_new = (y * mod_pow(a, x, N)) % N;
...
In simulation, the qubit count is the binding constraint, not the gate count. Factoring $N=15$ requires roughly $20$ qubits ($16\,\text{MB}$ of state-vector memory --- fits on a phone). Factoring $N=21$ requires $\sim 25$ qubits ($512\,\text{MB}$ --- fits on a laptop). Factoring a 32-bit number requires $\sim 160$ qubits, far beyond what state-vector simulation on \emph{any} conceivable classical hardware can reach. This is the asymptotic gap that motivated the entire field --- and the reason why simulation is fundamentally a tool for algorithm design and small-scale validation, not a substitute for a physical quantum computer.
